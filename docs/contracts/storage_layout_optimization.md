...
- `BidIndex(job_id, freelancer)` provides constant-key duplicate checks and accept validation.
...
- Dynamic bid keys are now allocated per submitted proposal, and bounded by `BidCount(job_id)`, minimizing unnecessary persistent data reads and rewrites.
