...
exports.isSqlSafe = isSqlSafe;
exports.sanitizeSqlInput = sanitizeSqlInput;
exports.sqlInjectionGuard = sqlInjectionGuard;
exports.findSqlInjectionInObject = findSqlInjectionInObject;
exports.validateQuerySafety = validateQuerySafety;
...
function sqlInjectionGuard(req, res, next) {
    ...
    if (req.query) {
        for (const [key, value] of Object.entries(req.query)) {
            if (typeof value === "string" && !isSqlSafe(value)) {
                tracing_1.logger.warn("SQL injection attempt detected in query params", {
                    key,
            ...
            if (Array.isArray(value)) {
                for (const item of value) {
                    if (typeof item === "string" && !isSqlSafe(item)) {
                        tracing_1.logger.warn("SQL injection attempt detected in query params array", {
                            key,
                            ip: req.ip,
                            path: req.originalUrl,
                        });
    ...
    if (req.body && typeof req.body === "object") {
        ...
        if (violations.length > 0) {
            tracing_1.logger.warn("SQL injection attempt detected in request body", {
                violations,
                ip: req.ip,
                path: req.originalUrl,
            });
 ...
 */
function findSqlInjectionInObject(obj, path = "", results = [], depth = 0) {
    if (depth > 10)
...
function validateQuerySafety(sql) {
    ...
    for (const pattern of concatPatterns) {
        if (pattern.test(sql)) {
            throw new Error("Potential SQL injection: query appears to use string concatenation instead of parameterized values");
        }
    }
}
