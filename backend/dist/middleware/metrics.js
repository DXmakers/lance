...
function metricsMiddleware(req, res, next) {
    ...
    metrics_1.httpActiveRequests.inc({ method });
    // Track request body size (if applicable)
    const contentLength = parseInt(req.headers["content-length"] || "0", 10);
    if (contentLength > 0) {
        metrics_1.httpRequestSize.observe({ method, path }, contentLength);
    }
    ...
    res.on("finish", () => {
        ...
        metrics_1.httpRequestDuration.observe({ method, path, status_code: statusCode }, durationMs);
        // Increment total request counter
        metrics_1.httpRequestsTotal.inc({ method, path, status_code: statusCode });
        // Decrement active request gauge
        metrics_1.httpActiveRequests.dec({ method });
        // Track response size from Content-Length header
        const responseLength = parseInt(res.getHeader("content-length") || "0", 10);
        if (responseLength > 0) {
            metrics_1.httpResponseSize.observe({ method, path, status_code: statusCode }, responseLength);
        }
    });
    next();
}
