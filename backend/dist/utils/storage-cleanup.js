...
exports.getStorageCleanupStats = getStorageCleanupStats;
exports.startStorageCleanup = startStorageCleanup;
exports.stopStorageCleanup = stopStorageCleanup;
const tracing_1 = require("../config/tracing");
const fs_1 = __importDefault(require("fs"));
const path_1 = __importDefault(require("path"));
const logger = tracing_1.trace.getLogger("storage-cleanup");
// ---------------------------------------------------------------------------
// Configuration
// ---------------------------------------------------------------------------
/** How often the cleanup job runs (ms). Default: 30 min */
const CLEANUP_INTERVAL_MS = parseInt(process.env.STORAGE_CLEANUP_INTERVAL_MS || (30 * 60 * 1000).toString(), 10);
/** Max age (ms) for temp upload files before they are considered stale. Default: 1 hour */
const STALE_FILE_AGE_MS = parseInt(process.env.STORAGE_STALE_FILE_AGE_MS || (60 * 60 * 1000).toString(), 10);
/** Disk usage warning threshold (percentage). Default: 80% */
const DISK_WARN_THRESHOLD = parseInt(process.env.STORAGE_DISK_WARN_PCT || "80", 10);
/** Disk usage critical threshold (percentage). Default: 90% */
const DISK_CRITICAL_THRESHOLD = parseInt(process.env.STORAGE_DISK_CRITICAL_PCT || "90", 10);
...
let filesCleaned = 0;
let bytesFreed = 0;
function getStorageCleanupStats() {
...
function getDiskUsage() {
    try {
        const stats = fs_1.default.statfsSync(TEMP_DIR);
        return {
...
async function cleanStaleTempFiles() {
    ...
    try {
        ...
        const entries = fs_1.default.readdirSync(TEMP_DIR, { withFileTypes: true });
        for (const entry of entries) {
            if (entry.isFile()) {
                const filePath = path_1.default.join(TEMP_DIR, entry.name);
                try {
...
async function checkDiskUsage() {
    try {
        const { size, available } = getDiskUsage();
        if (size === 0)
            return; // Cannot determine disk usage
 ...
 */
async function cleanStaleUploadRecords() {
    try {
