...
const submitEvidenceSchema = zod_1.z.object({
    submitted_by: zod_1.z.string().min(1),
    content: zod_1.z.string().optional().default(""),
    file_hash: zod_1.z.string().optional().nullable(),
});
const createAppealSchema = zod_1.z.object({
    requester_address: zod_1.z.string().min(1),
});
...
router.get("/:id", async (req, res) => {
    try {
        const { id } = req.params;
        const dispute = await db_1.prisma.disputes.findUnique({
            where: { id },
        });
        if (!dispute) {
...
router.get("/:id/evidence", async (req, res) => {
    try {
        ...
        const evidence = await db_1.prisma.evidence.findMany({
...
router.post("/:id/evidence", async (req, res) => {
    try {
        ...
        const evidence = await db_1.prisma.evidence.create({
            data: {
...
router.get("/:id/verdict", async (req, res) => {
    try {
        const { id: disputeId } = req.params;
        const verdict = await db_1.prisma.verdicts.findFirst({
            where: { dispute_id: disputeId },
...
router.post("/:id/appeal", async (req, res) => {
    try {
        ...
        const existing = await db_1.prisma.appeals.findUnique({
            where: { dispute_id: disputeId },
        });
        if (existing) {
            return res.status(400).json({ error: "an appeal already exists for this dispute" });
        }
        const appeal = await db_1.prisma.appeals.create({
            data: {
                dispute_id: disputeId,
                status: "open",
            },
