use crate::{metrics::Metrics, models::IndexedEventRecord, repository::Repository, rpc::StellarRpcClient};
...
pub async fn run_once(context: &WorkerContext) -> anyhow::Result<()> {
    ...
    if latest < checkpoint {
        warn!(checkpoint, latest, "rpc returned a ledger lower than the stored checkpoint");
    }
...
mod tests {
    ...
    async fn worker_recovers_after_rpc_failures() -> anyhow::Result<()> {
        ...
        Mock::given(method("POST")).and(body_string_contains("\"method\":\"getLatestLedger\""))
            .respond_with(ResponseTemplate::new(200).set_body_json(&latest_response))
            .mount(&mock_server)
            .await;

        let ledger1_payload = json!({"sequence": 1, "data": "ledger1"});
        Mock::given(method("POST")).and(body_string_contains("\"method\":\"getLedger\""))
            .and(body_string_contains("\"sequence\": 1"))
            .respond_with(ResponseTemplate::new(200).set_body_json(json!({"result": ledger1_payload, "error": null})))
            .mount(&mock_server)
            .await;
        ...
        let snapshot = std::sync::Arc::new(tokio::sync::RwLock::new(crate::models::SyncSnapshot::new(0)));
        ...
        assert!(res.is_err(), "expected run_once to return error due to ledger 2 failure");

        let checkpoint = repo.load_checkpoint().await?;
        assert_eq!(checkpoint, 1, "checkpoint should have advanced to 1 after partial processing");
        ...
        Mock::given(method("POST")).and(body_string_contains("\"method\":\"getLedger\""))
            .and(body_string_contains("\"sequence\": 2"))
            .respond_with(ResponseTemplate::new(200).set_body_json(json!({"result": ledger2_payload, "error": null})))
            .mount(&mock_server)
            .await;

        // Second run should succeed fully
        let res2 = run_once(&context).await;
        assert!(res2.is_ok(), "expected second run_once to succeed after RPC recovery");

        let checkpoint2 = repo.load_checkpoint().await?;
        assert_eq!(checkpoint2, 2, "checkpoint should be 2 after processing ledger 2");

        // Verify an indexed event for ledger:2 exists
        let found: Option<i64> = sqlx::query_scalar("SELECT id FROM indexed_events WHERE event_key = $1")
            .bind("ledger:2")
            .fetch_optional(&pool)
            .await?;
