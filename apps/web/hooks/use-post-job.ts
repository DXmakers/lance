  ...
  const submit = useCallback(
    async (input: PostJobInput) => {
      ...
      try {
        ...
        const onStep: LifecycleListener = (step, detail, payload) => {
          setStep(step, detail);

          if (payload?.rawXdr) {
            useTxStatusStore.getState().setRawXdr(payload.rawXdr);
          }
          if (payload?.retryCount !== undefined) {
            useTxStatusStore.getState().setRetryCount(payload.retryCount);
          }
