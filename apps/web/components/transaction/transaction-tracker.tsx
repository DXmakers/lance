...
export function TransactionTracker() {
  const { step, detail, txHash, simulation, startedAt, finishedAt, retryCount, rawXdr } =
    useTxStatusStore();
  ...
  return (
    <div className="rounded-[2rem] border border-slate-200 bg-white/85 p-6 shadow-[0_25px_80px_-48px_rgba(15,23,42,0.5)]">
      {/* ── Progress & Status Extra ────────────────────────────────────── */}
      {retryCount > 0 && !isConfirmed && !isFailed && (
        <div className="mb-4 inline-block rounded-xl border border-amber-500/20 bg-amber-500/10 px-3 py-1.5 text-xs font-mono font-medium text-amber-600">
          State Stale: Retrying Transaction (Attempt {retryCount})
        </div>
      )}
      ...
      {rawXdr && !isFailed && !isConfirmed && (
        <div className="mt-4 rounded-xl border border-slate-200 bg-slate-900 p-4">
          ...
          <div className="max-h-24 overflow-y-auto break-all font-mono text-[10px] text-slate-300">
            {rawXdr}
          </div>
        </div>
      )}
